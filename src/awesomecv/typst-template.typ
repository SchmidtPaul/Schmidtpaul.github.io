...
#let resume-entry(
  ...
  pad[
    // Check if location has actual content (not just empty brackets)
    #let has-location = if type(location) == content {
      repr(location) != "[]"
    } else {
      location != "" and location != none
    }

    // Check if date has actual content
    #let has-date = if type(date) == content {
      repr(date) != "[]"
    } else {
      date != "" and date != none
    }
    ...
    #let has-description = if type(description) == content {
      repr(description) != "[]"
    } else {
      description != "" and description != none
    }

    // Only render headers if there's content
    #if title != none or has-location [
      #justified-header(title, location)
    ]

    #if has-description or has-date [
